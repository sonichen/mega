use crate::{
    ...
    extract_text_metadata, get_file_type, Config, DataType, Entry, Header, ImageMetaData, MDAIndex,
    Revlog, TextMetaData, TrainData, TrainingData,
};
use anyhow::Result;
...
/// Get annotation data
pub fn config_annotation_data(file_path: &str) -> Result<Revlog, Box<dyn Error>> {
    let mut file = File::open(file_path)?;

    let mut content = String::new();
    file.read_to_string(&mut content)?;
    let (headers, entries) = Entry::init(&content);
...
pub fn write_data_to_mda(
    ...
    for (revlog_header, &length) in revlog.headers.iter_mut().zip(lengths.iter()) {
        revlog_header.offset = anno_entries_offset;
        revlog_header.length = length;
        let header_bytes = bincode::serialize(revlog_header)?;
